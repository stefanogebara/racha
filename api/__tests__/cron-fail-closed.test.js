...
const SAIDA_SEM_PORTAO = {
  ...
  '/api/pay': (
    ...
    + 'um teto em DUAS camadas — `exigeVagaDaOrigem` (conta × IP, o que uma mesa '
    + 'legítima gasta) e `assertChargeSlot` (por conta, com reserva em voo, alto o '
    + 'bastante pra que uma origem só não o encha). A primeira versão tinha UMA '
    + 'camada, e a revisão de segurança de 2026-09-15 mediu que ela estava ao '
    + 'contrário: não parava um script concorrente e dava a um script serial uma '
    + 'negação de serviço contra a mesa inteira, com sessenta cobranças de um '
    + 'centavo. O QUE CONTINUA ABERTO: (1) numa rota de token portador, todo '
    + 'recurso por conta é esgotável por quem tem o token — com várias origens, '
    + 'o teto por conta enche e a mesa leva 429 por quinze minutos; o remédio que '
    + 'a tela dá é esperar ou fechar no caixa; (2) entre instâncias o teto vira '
    + 'teto vezes instâncias, porque a reserva em voo é local — fechar isso exige '
    + 'um RPC que conte e reserve numa instrução; (3) não há idempotência no '
    + 'adquirente — só o MockPsp colapsa repetição, derivando o `txid` do '
    + '`chargeRef`; (4) e a consequência REAL de uma enxurrada não é o volume de '
    + 'chamadas da conciliação, que já é limitado por varredura: é a FOME dela. '
    + 'O `listPendingCharges` ordena da mais velha pra mais nova e a varredura '
    + 'pega um lote fixo, então uma pilha de pendentes de ataque na janela de '
    + '24 horas empurra as cobranças legítimas recentes pra fora da varredura, e '
    + 'elas perdem a rede de segurança do webhook perdido.'
  ),
  '/api/pay/stripe-intent': (
    ...
    + '`payerLabel`. Por isso as duas camadas do teto são funções chamadas nos '
    + 'dois sítios (`exigeVagaDaOrigem` e `assertChargeSlot`), com testes '
    + 'estruturais exigindo as duas, em vez de uma terceira cópia da regra. A '
    + 'forma do ERRO também é a mesma agora: esta rota tinha catch próprio que '
    + 'devolvia a frase interna sem código, e passou a sair pelo `errorBody` com '
    + '`Retry-After`. Valem as quatro ressalvas da rota irmã.'
  ),
  '/api/house/load': (
    'CARGA DE SALDO DA CASA: também cria cobrança, e é a MENOS gateada das três '
    + 'por outro caminho — exige o `accountToken` de uma conta de saldo em vez '
    + 'do token da mesa, mas contas são de graça: o `openAccount` aceita qualquer '
    + 'telefone de 10 a 13 dígitos sem verificação. (A versão anterior desta '
    + 'linha a chamava de "a mais exposta"; a mais exposta é o `/api/pay`, cujo '
    + 'token viaja em foto de QR.) O serviço confere o portão de mercado do '
    + 'saldo da casa e os limites de valor da configuração, e o `assertLoadSlot` '
    + 'aplica teto por CONTA e por CASA — este último porque um teto só por conta '
    + 'num endpoint em que contas são geradas é um teto sobre nada. O preço, '
    + 'dito: vinte contas esgotam as recargas daquela casa por quinze minutos. '
    + 'O `chargeRef` daqui leva um `randomUUID` de propósito, então não há '
    + 'idempotência nem nominal.'
  ),
};
