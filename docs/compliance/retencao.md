...
| `charge_slots` — o livro de vagas do teto de cobranças (migração 0033) | cada linha carrega a própria janela — **15 minutos** nas de cobrança e recarga, 6 horas nas de aviso, um dia nos contadores de aviso — e o expurgo diário apaga a que passou dela: as de cobrança e recarga (onde está o id de conta) ficam **até cerca de 24 horas**; as de aviso, sem dado pessoal, até cerca de 48 | Controle de abuso: impedir que um token de mesa fotografado emita cobranças sem limite. Guarda só chaves (`check:`/`account:`/`venue:`/`alerta:`/`alerta-dia:`), e o id de conta de saldo é pseudônimo — por isso tem prazo, e curto. Ver o teste de balanceamento no `data-map.md`. | a própria reivindicação (a chave dela, e lotes de linhas com mais de um dia) e `purge_expired_personal_data()`, diário |
