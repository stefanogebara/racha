...
| `check_events` — o razão da conta, inclusive a nota e o autor (`by`, o **id de usuário**) de um `PAYMENT_ISSUE_RESOLVED` e a `reference` + `by` de uma devolução registrada fora do trilho (`record-restitution`) | **5 anos do exercício**, com o pagamento | O razão é a fonte do estado do dinheiro (inegociável #6) e a defesa numa disputa: não se apaga linha dele. O risco é o TEXTO LIVRE, em dois campos: o painel manda texto fixo pro pago-depois-de-fechar, mas a rota `resolve-issue` chamada à mão e a `reference` da devolução fora do trilho aceitam o que quem opera escrever — não escreva nome, telefone, CPF ou chave Pix do cliente em nenhum dos dois (o id E2E do Pix basta como prova). O autor vai como id de usuário, nunca e-mail (minimização, art. 6º III). | ninguém: só-de-acréscimo — **lacuna**, ver o data-map |
