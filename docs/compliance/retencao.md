...
| `check_events` — o razão da conta, inclusive a nota e o autor de um `PAYMENT_ISSUE_RESOLVED` | **5 anos do exercício**, com o pagamento | O razão é a fonte do estado do dinheiro (inegociável #6) e a defesa numa disputa: não se apaga linha dele. O risco é a nota LIVRE: o painel manda texto fixo pro pago-depois-de-fechar, e a rota `resolve-issue` à mão ainda aceita texto — não escreva nome, telefone ou chave Pix do cliente nela. | ninguém: só-de-acréscimo — **lacuna**, ver o data-map |
