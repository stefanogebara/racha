...
| `charge_slots` — o livro de vagas do teto de cobranças (migração 0033) | a contagem olha **15 minutos**; a linha fica **no máximo 1 dia** | Controle de abuso: impedir que um token de mesa fotografado emita cobranças sem limite. Guarda só chaves (`check:`/`account:`/`venue:`/`alerta:`), e o id de conta de saldo é pseudônimo — por isso tem prazo, e curto. Ver o teste de balanceamento no `data-map.md`. | a própria reivindicação (a chave dela, e lotes de linhas com mais de um dia) e `purge_expired_personal_data()`, diário |
