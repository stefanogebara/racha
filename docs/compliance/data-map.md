...
| **Provedor de e-mail do auth** — **Resend** (empresa nos EUA; envio pela região **São Paulo, sa-east-1**, AWS SES), SMTP no projeto do Racha. Remetente `Racha <nao-responda@useracha.app>` desde 2026-09-25, no domínio PRÓPRIO (SPF/DKIM/DMARC verificados; `docs/domains.md`). Chave só de envio e só desse domínio; a da ponte em `seatable.one` foi REVOGADA. Sem rastreio de clique/abertura (o link do e-mail chega intacto — conferido). Teste real: redefinição em pt chegou na CAIXA DE ENTRADA do Gmail, sem aviso. DPA do Resend a anexar (lacuna 4) | e-mail do dono, o idioma dele, e o link de confirmação ou de redefinição de senha | confirmar o cadastro e redefinir a senha | a definir junto com o provedor — é operador novo |
