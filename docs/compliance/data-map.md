...
| `check_events.payload.note` + `.by` de um `PAYMENT_ISSUE_RESOLVED` — quem encerrou uma pendência de dinheiro e por quê | dono (o e-mail dele, em `by`) e, se ele escrever, quem pagou | registrar quem encerrou a pendência e por quê — é o que prova, depois, que a marca não sumiu sozinha | operadora, por conta da casa (art. 7º V da casa) | Supabase `check_events`, o razão só-de-acréscimo | **segue o razão (5 anos do exercício)** — e é **lacuna**: não há ferramenta de apagar dentro de um razão imutável. Por isso a resposta do painel ao pago-depois-de-fechar é **texto fixo, escrito pelo servidor** ("a mesa não pagou no caixa"); só a rota `resolve-issue` chamada à mão, pra falha de estorno, ainda aceita texto livre |
