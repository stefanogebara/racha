...
| **Resend** (suboperador da Olímpia) | o TEXTO do alerta de fundador: `txid`, `checkId`, valor, e nomes de casa em achado de conciliação; no **aviso de teto disparado**, nome da casa, rótulo da mesa e `checkId` (dado de empresa e id pseudônimo, sem dado do cliente) | entregar o alerta por e-mail | EUA |
...
**O livro de vagas do teto (`charge_slots`, migração 0033).** Uma linha por **cobrança em que o adquirente foi chamado** — duas numa recarga de saldo (uma pela casa, uma pela conta) — mais uma linha de alerta por conta ou casa cujo teto disparou e um contador diário de alertas. A linha **fica além dos quinze minutos** da contagem: até a mesma chave ser reivindicada de novo ou até o expurgo diário. Chaves: `check:<id>` com a geração do QR (hash de um token aleatório, não o token), `account:<id>`, `venue:<id>`, `alerta:check:<id>` — **nenhum IP, telefone ou nome**. O id de conta de saldo é **pseudônimo, e é dado pessoal**. **Prazo:** a contagem só olha quinze minutos; a linha fica até a mesma chave ser reivindicada de novo ou até o **expurgo diário** (`purge_expired_personal_data`, que apaga tudo que passou da janela e pagina se parar) — **até cerca de 24 horas**; ver `retencao.md`. **Base:** legítimo interesse de prevenção a fraude e abuso (arts. 7º IX e 10). O teste de balanceamento, pelo mesmo critério pedido ao `check_views`: a finalidade é impedir que um token de mesa fotografado emita cobranças sem limite contra o recebedor da casa; o dado é o mínimo que a conta exige (um id, sem conteúdo); a expectativa do titular é razoável (é o mesmo id que já está na cobrança dele); e o prazo é o menor que o controle permite. A alternativa que separaria um atacante da mesa — guardar uma chave de ORIGEM, ainda que em hash de IP — foi considerada e **não** adotada, e não pela LGPD, que a permitiria: ela não funcionaria onde importa (no wi-fi do salão atacante e mesa são o mesmo NAT; no NAT das operadoras móveis um IP novo sai no modo avião).
