...
| `house_accounts.phone` + `.name` | cliente que abre carteira pré-paga | achar a carteira dele na casa e saber de quem é o saldo; e, desde 2026-09-26, a casa **avisar o cliente de movimentação no saldo dele** (o número inteiro sai pro dono só na resposta de "devolver ao saldo", pra ele avisar pelo WhatsApp — dito no aviso de privacidade, `priv.what3`) | operadora | Supabase `house_accounts` | enquanto a carteira viver, **+ 90 dias** depois de zerada e inativa |
...
| **Caixa de contato** (`contato@useracha.app`, desde 2026-09-26) — **Forward Email, Inc.** (EUA; plano grátis, só DNS; encaminha em memória, sem gravar conteúdo nem metadados segundo a política deles; DPA existe, NÃO conferido se cobre o plano grátis) → **Gmail pessoal do dono** (Google LLC, EUA; **termos de consumidor, sem DPA nem papel de operador** — o elo fraco) | e-mail, nome e o texto de quem escreve: pedidos do art. 18 e demandas de consumidor (pode vir rótulo de pagamento, CPF, telefone) | receber e responder pedido de titular e ser o canal do Decreto 7.962 art. 2º II | EUA — **transferência internacional (art. 33)**. Base registrada: **art. 33 IX c/c art. 7º II** — responder ao art. 18 é obrigação legal, e a caixa existe pra isso. Fallback aceitável só pro piloto assistido: **antes do self-serve**, trocar por caixa com termos de operador (Google Workspace + Cloud Data Processing Addendum, ou caixa paga do Forward Email com DPA). Resposta: desde 2026-09-26 sai COMO `contato@` pelo SMTP do **Resend** (já operador, linha acima), com chave só de envio; a resposta automática ainda sai do Gmail pessoal — ver `docs/runbooks/contato.md`. Retenção: `retencao.md` |
| **WhatsApp (Meta), pelo aparelho do DONO** (`wa.me`, desde 2026-09-26) | o telefone do cliente da carteira, o nome dele, o nome da casa e o valor devolvido, numa mensagem pronta | o dono avisar o cliente de que o saldo voltou (0044, compliance PR #44 M-3) — CDC art. 6º III | **não é operador da Racha**: o servidor só devolve o número inteiro ao dono naquela resposta (registrado no log, sem o número). **Ao tocar "Avisar", o número e o texto vão à Meta (`wa.me`, depois `api.whatsapp.com`/WhatsApp Web) pelo navegador do dono** — mesmo que ele não aperte "enviar" no WhatsApp —, e ficam no histórico do navegador dele. O ENVIO da mensagem é do dono, controlador do dado do cliente da carteira, pela conta de WhatsApp dele (Meta, EUA — a transferência do art. 33 é da casa). A mensagem sai sempre em português (CDC art. 31) |
